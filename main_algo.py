...
def get_ArgumentParser():
    ...
    parser.add_argument("--LOG_FNAME", default="_mnmf.log")
    ...
    parser.add_argument("--ZETA", default=1e+9, help="L2 regularization weight for orthogonality constraint")
...
def main():
    ...
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    S = du.get_proximity_similarity_matrix(dataset.relations, float(config.ETA))
    ...
    for a in perc_data :
        ...
        for b in range(1, 1 + 1): # Running for only one fold
            ...
            logger.debug("Iter# {} LR_Micro_F1: {} SVM_Micro_F1: {} ".format(best_result_lr['i'], best_lr_accu['micro_f1'], best_svm_accu['micro_f1']))
            print("Iter# {} LR_Micro_F1: {} SVM_Micro_F1: {} ".format(best_result_lr['i'], best_lr_accu['micro_f1']*100, best_svm_accu['micro_f1']*100))
            itr += 1
            if config.SAVE_EMB:
        ...
        with open("tmp_output_files/"+str(config.DATA_DIR) + "_" + config.TYPE+ "_" + str(a) + "_" + "best_params_nc.txt", 'wb') as fp:
            pkl.dump({"LR":temp1, "SVM":temp2}, fp)
